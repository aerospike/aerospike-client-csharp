...
namespace Aerospike.Test
{
	public class TestOperateMap : TestSync
	{
		...
		public void OperateMapPut()
		{
			...
			Xunit.Assert.Equal(4, map.Count);
			Xunit.Assert.Equal(1L, map[10L]);
		}
		...
		public void OperateMapPutItems()
		{
			...
			Xunit.Assert.Equal("my default", str);

			str = (string)results[i++];
			Xunit.Assert.Equal("changed", str);
		...
		public void OperateMapSwitch()
		{
			...
			Xunit.Assert.Equal(3L, entry.Key);
			Xunit.Assert.Equal(3L, entry.Value);

			list = (IList)results[i++];
			Xunit.Assert.Equal(4, list.Count);
			entry = (KeyValuePair<object,object>)list[1];
			Xunit.Assert.Equal(2L, entry.Key);
			...
			Xunit.Assert.Equal(1L, entry.Value);

			list = (IList)results[i++];
			entry = (KeyValuePair<object,object>)list[3];
			Xunit.Assert.Equal(4L, entry.Key);
		}
		...
		public void OperateMapRank()
		{
			...
			str = (string)list[0];
			Xunit.Assert.Equal("Harry", str);
			str = (string)list[1];
			Xunit.Assert.Equal("Jim", str);

			list = (IList)results[i++];
			KeyValuePair<object,object> entry = (KeyValuePair<object,object>)list[0];
			Xunit.Assert.Equal("Charlie", entry.Key);
			Xunit.Assert.Equal(55L, entry.Value);
			entry = (KeyValuePair<object,object>)list[1];
			Xunit.Assert.Equal("John", entry.Key);
			Xunit.Assert.Equal(81L, entry.Value);

			val = (long)results[i++];
			Xunit.Assert.Equal(55, val);
			...
			list = (IList)results[i++];
			entry = (KeyValuePair<object,object>)list[0];
			Xunit.Assert.Equal("Jim", entry.Key);
			Xunit.Assert.Equal(94L, entry.Value);

			list = (IList)results[i++];
			str = (string)list[0];
			Xunit.Assert.Equal("John", str);
		...
		public void OperateMapRemove()
		{
			...
			long val = (long)results[i++];
			Xunit.Assert.Equal(7, val);

			object obj = results[i++];
			Xunit.Assert.Null(obj);

			val = (long)results[i++];
			Xunit.Assert.Equal(98, val);
			...
			IList list = (IList)results[i++];
			Xunit.Assert.Equal(1, list.Count);
			Xunit.Assert.Equal("Charlie", (string)list[0]);
		...
		public void OperateMapScore()
		{
			...
			Xunit.Assert.Equal("weiling", s);

			list = (IList)results[i++];
			s = (string)list[0];
			Xunit.Assert.Equal("brianb", s);
			s = (string)list[1];
			Xunit.Assert.Equal("briann", s);
			s = (string)list[2];
			Xunit.Assert.Equal("meher", s);
		}	
	}
}
