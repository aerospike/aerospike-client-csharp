...
{
	public class AsyncUDFInit : TestAsync
	{
		public AsyncUDFInit()
		{
			Assembly assembly = typeof(AsyncUDFInit).GetTypeInfo().Assembly;
			RegisterTask task = client.Register(null, assembly, "AerospikeTest.record_example.lua", "record_example.lua", Language.LUA);
			task.Wait();
		}
	}

	public class TestAsyncUDF : TestAsync, Xunit.IClassFixture<AsyncUDFInit>
	{
		private static readonly string binName = args.GetBinName("audfbin1");
		private const string binValue = "string value";

		AsyncUDFInit fixture;

		public TestAsyncUDF(AsyncUDFInit fixture)
		{
