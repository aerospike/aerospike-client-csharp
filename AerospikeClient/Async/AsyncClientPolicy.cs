...
namespace Aerospike.Client
{
	...
	public sealed class AsyncClientPolicy : ClientPolicy
	{
		...
		/// Concurrent commands can be to different nodes of the Aerospike cluster. Each command 
		/// will use one concurrent connection.
		/// The number of concurrent open connections is therefore limited by:
		/// <para>
		/// max open connections = asyncMaxCommands
		/// </para>
		/// The actual number of open connections to each node of the Aerospike cluster depends on how balanced the commands are 
		/// between nodes and are limited to asyncMaxConnsPerNode for any given node. For an extreme case where all commands
		/// may be destined to the same node of the cluster, asyncMaxCommands should not be set greater than asyncMaxConnsPerNode to 
		/// avoid running out of connections to the node.
		/// <para>
		/// Further, this maximum number of open connections across all nodes 
		/// should not exceed the total socket file descriptors available
