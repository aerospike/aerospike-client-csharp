...
{
	internal class WriteCommandNew : ICommand
	{
		...
		public WriteCommandNew(ArrayPool<byte> bufferPool, Cluster cluster, WritePolicy writePolicy, Key key, Bin[] bins, Operation.Type operation)
		{
		...
		public WriteCommandNew(ArrayPool<byte> bufferPool, Cluster cluster, WritePolicy writePolicy, Key key)
		{
			this.SetCommonProperties(bufferPool, cluster, writePolicy);
			this.writePolicy = writePolicy;
			this.key = key;
		...
		{
			this.SetWrite(writePolicy, operation, key, bins);
		}
