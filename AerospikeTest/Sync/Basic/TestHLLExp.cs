	...
	public class TestHLLExp : TestSync
	{
		...
		public async Task HllExp()
		{
			...
			if (!args.testAsyncAwait)
			{
				...
				await Union(key);
				await Intersect(key);
				await Similarity(key);
				await Describe(key);
				await MayContain(key);
				await Add(key);
			}
			else
			{
				...
				Record rec = await asyncAwaitClient.Operate(null, key,
					new[] { HLLOperation.Add(HLLPolicy.Default, bin1, list1, 8),
					HLLOperation.Add(HLLPolicy.Default, bin2, list2, 8),
					HLLOperation.Add(HLLPolicy.Default, bin3, list3, 8),
					Operation.Get(bin1),
					Operation.Get(bin2),
		...
		private async Task Union(Key key)
		{
		...
		private async Task Intersect(Key key)
		{
		...
		private async Task Similarity(Key key)
		{
			List<Value.HLLValue> hlls2 = new List<Value.HLLValue>();
			hlls2.Add(hll2);
		...
		private async Task Describe(Key key)
		{
			Exp index = Exp.Val(0);
		...
		private async Task MayContain(Key key)
		{
