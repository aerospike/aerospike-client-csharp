...
namespace Aerospike.Test
{
	...
	public class TestOperateList : TestSync
	{
		...
		[TestMethod]
		public async Task OperateList1()
		{
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.Append(binName, Value.Get(55)),
					ListOperation.Append(binName, Value.Get(77)),
					ListOperation.Pop(binName, -1),
					ListOperation.Size(binName) },
		...
		[TestMethod]
		public async Task OperateList2()
		{
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.AppendItems(binName, itemList),
					Operation.Put(new Bin("otherbin", "hello")) },
					CancellationToken.None
					);
				...
				record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.Insert(binName, -1, Value.Get(8)),
					Operation.Append(new Bin("otherbin", Value.Get("goodbye"))),
					Operation.Get("otherbin"),
					ListOperation.GetRange(binName, 0, 4),
					ListOperation.GetRange(binName, 3) },
		...
		[TestMethod]
		public async Task OperateList3()
		{
		...
		[TestMethod]
		public async Task OperateList4()
		{
		...
		[TestMethod]
		public async Task OperateList5()
		{
			// Test trim.
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.InsertItems(binName, 0, itemList),
					ListOperation.Trim(binName, -5, 5),
					ListOperation.Trim(binName, 1, -5),
					ListOperation.Trim(binName, 1, 2) },
					//ListOperation.Trim(binName, 11, 6) causes entire command to fail.
		...
		[TestMethod]
		public async Task OperateList6()
		{
			// Test clear.
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(policy, key,
					new[] { Operation.Put(new Bin("otherbin", 11)),
					Operation.Get("otherbin"),
					ListOperation.AppendItems(binName, itemList),
					ListOperation.Clear(binName),
					ListOperation.Size(binName) },
		...
		[TestMethod]
		public async Task OperateList7()
		{
			// Test null values.
		...
		[TestMethod]
		public async Task OperateList8()
		{
			// Test null values.
		...
		[TestMethod]
		public async Task OperateListSwitchSort()
		{
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.AppendItems(ListPolicy.Default, binName, itemList),
					ListOperation.GetByIndex(binName, 3, ListReturnType.VALUE) },
					CancellationToken.None
					);
				...
				// Sort list.
				record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.SetOrder(binName, ListOrder.ORDERED),
					ListOperation.GetByValue(binName, Value.Get(3), ListReturnType.INDEX),
					ListOperation.GetByValueRange(binName, Value.Get(-1), Value.Get(3), ListReturnType.COUNT),
					ListOperation.GetByValueRange(binName, Value.Get(-1), Value.Get(3), ListReturnType.EXISTS),
					ListOperation.GetByValueList(binName, valueList, ListReturnType.RANK),
		...
		[TestMethod]
		public async Task OperateListSort()
		{
		...
		[TestMethod]
		public async Task OperateListRemove()
		{
		...
		[TestMethod]
		public async Task OperateListInverted()
		{
		...
		[TestMethod]
		public async Task OperateListGetRelative()
		{
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.AppendItems(new ListPolicy(ListOrder.ORDERED, ListWriteFlags.DEFAULT), binName, itemList),
					ListOperation.GetByValueRelativeRankRange(binName, Value.Get(5), 0, ListReturnType.VALUE),
					ListOperation.GetByValueRelativeRankRange(binName, Value.Get(5), 1, ListReturnType.VALUE),
					ListOperation.GetByValueRelativeRankRange(binName, Value.Get(5), -1, ListReturnType.VALUE),
					ListOperation.GetByValueRelativeRankRange(binName, Value.Get(3), 0, ListReturnType.VALUE),
		...
		[TestMethod]
		public async Task OperateListRemoveRelative()
		{
		...
		[TestMethod]
		public async Task OperateListPartial()
		{
			...
			else
			{
				...
				Record record = await asyncAwaitClient.Operate(null, key,
						new[] { ListOperation.AppendItems(new ListPolicy(ListOrder.ORDERED, ListWriteFlags.ADD_UNIQUE | ListWriteFlags.PARTIAL | ListWriteFlags.NO_FAIL), binName, itemList),
						ListOperation.AppendItems(new ListPolicy(ListOrder.ORDERED, ListWriteFlags.ADD_UNIQUE | ListWriteFlags.NO_FAIL), "bin2", itemList) },
						CancellationToken.None
						);
		...
		[TestMethod]
		public async Task OperateListInfinity()
		{
			...
			else
			{
				...
				record = await asyncAwaitClient.Operate(null, key,
						new[] { ListOperation.GetByValueRange(binName, Value.Get(10), Value.INFINITY, ListReturnType.VALUE) },
						CancellationToken.None
						);
		...
		[TestMethod]
		public async Task OperateListWildcard()
		{
			...
			else
			{
				...
				record = await asyncAwaitClient.Operate(null, key,
						new[] { ListOperation.GetByValue(binName, Value.Get(itemList), ListReturnType.VALUE) },
						CancellationToken.None
						);
		...
		[TestMethod]
		public async Task OperateNestedList()
		{
			...
			else
			{
				...
				// Create list.
				await asyncAwaitClient.Put(null, key, new[] { new Bin(binName, inputList) }, CancellationToken.None);

				// Append value to last list and retrieve all lists.
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.Append(binName, Value.Get(11), CTX.ListIndex(-1)),
					Operation.Get(binName) },
					CancellationToken.None
					);
		...
		[TestMethod]
		public async Task OperateNestedListMap()
		{
			...
			else
			{
				...
				// Append value to last list and retrieve map.
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.Append(binName, Value.Get(11), CTX.MapKey(Value.Get("key2")), CTX.ListRank(0)),
					Operation.Get(binName) },
					CancellationToken.None
					);
		...
		[TestMethod]
		public async Task OperateListCreateContext()
		{
			...
			else
			{
				...
				// Create list.
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.AppendItems(new ListPolicy(ListOrder.ORDERED, 0), binName, inputList),
					Operation.Get(binName) },
					CancellationToken.None
					);
				...
				// Append value to new list created after the original 3 lists.
				record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.Append(binName, Value.Get(2), CTX.ListIndexCreate(3, ListOrder.ORDERED, false)),
					//ListOperation.Create(binName, ListOrder.ORDERED, false, CTX.ListIndex(3)),
					//ListOperation.Append(binName, Value.Get(2), CTX.ListIndex(3)),
					Operation.Get(binName) },
					CancellationToken.None
		...
		[TestMethod]
		public async Task OperateListBounded()
		{
			...
			else
			{
				...
				// Create list.
				Record record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.AppendItems(binName, inputList) },
					CancellationToken.None
					);
				...
				// Insert values to new list that are in bounds.
				record = await asyncAwaitClient.Operate(null, key,
					new[] { ListOperation.Insert(listPolicy, binName, 1, Value.Get(22)),  // Insert at index 1.
					ListOperation.Insert(listPolicy, binName, -1, Value.Get(44)), // Insert at last offset in list.
					Operation.Get(binName) },
					CancellationToken.None
					);
				...
				{
					record = await asyncAwaitClient.Operate(null, key,
						new[] { ListOperation.Insert(listPolicy, binName, 6, Value.Get(20)),
						Operation.Get(binName) },
						CancellationToken.None
						);
					//results = record.GetList(binName);
